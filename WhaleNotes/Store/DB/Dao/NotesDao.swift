...
class NotesDao {
    ...
    func insert( _ note:Note2) throws -> Int64 {
...
extension NotesDao {
    ...
    fileprivate func generateNoteInsert(note: Note2,conflict:OnConflict = OnConflict.fail) -> Insert {
    ...
    fileprivate func generateNote(row: Row) -> Note2 {
        let note = Note2(id: row[Field_Blocks.id], createdAt: row[Field_Blocks.createdAt], updatedAt: row[Field_Blocks.createdAt])
        return note
    }
}
